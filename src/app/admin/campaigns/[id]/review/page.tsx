...
export default function CampaignReview({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const [decision, setDecision] = useState('Pending Review');
  const [message, setMessage] = useState('');
  const decide = (value: string) => { setDecision(value); setMessage(`Campaign ${id} marked as ${value}.`); };

  return (
    <AppShell role="admin" title="Campaign Review">
      <Breadcrumbs items={[{ label: 'Campaigns', href: '/admin/campaigns' }, { label: id, href: `/admin/campaigns/${id}` }, { label: 'Review' }]} />
      ...
      <div className="rounded-xl border bg-white p-5 shadow-sm">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link href={`/admin/campaigns/${id}`} className="rounded-md border px-3 py-2 text-sm">Back to Detail</Link>
        </div>
      </div>
    </AppShell>
  );
