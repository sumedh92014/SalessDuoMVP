...
export default function CampaignDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const set = (next: string) => { setStatus(next); setMessage(`Campaign ${id} set to ${next}.`); };
  ...
  return (
    <AppShell role="admin" title="Campaign Detail">
      ...
      <div className="rounded-xl border bg-white p-5 shadow-sm space-y-4">
        <div><h2 className="text-xl font-semibold">Campaign {id}</h2><p className="text-sm text-slate-600">Status: <strong>{status}</strong></p></div>
        <div className="flex flex-wrap gap-2">
          ...
          <Link href={`/admin/campaigns/${id}/review`} className="rounded-md bg-violet-600 px-3 py-2 text-sm text-white">Open Review</Link>
