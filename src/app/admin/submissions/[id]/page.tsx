...
export default function SubmissionDetail({ params }: { params: { id: string } }) {
  ...
  const act = (next: string) => { setStatus(next); setMessage(`Submission ${params.id} set to ${next}.`); };

  return (
    <AppShell role="admin" title="Submission Detail">
      <Breadcrumbs items={[{ label: 'Submissions', href: '/admin/submissions' }, { label: params.id }]} />
